...
- 全部 `✅ PASS` **且每行都带 `🔴 RED`** → 继续
- 有未完成 → 提示 "还有 N 个测试未完成，请先 /openflow build"
- 有 `✅ PASS` 但缺 `🔴 RED` → 提示 "N 个测试从未被观察到失败过，这些绿灯不算证据，请先 /openflow build 按 Step 2 补 RED"。**不要替用户补标记**——没跑过就没有证据，补标记等于伪造
...
- **改动点归属对账（闸门 3 扩展）**：
  - **必须先跑**（gate helper 路径定位见主 SKILL.md「状态检测 → Helpers 定位」；脚本缺失或报错 → 停止并要求重装，不做手动替代）：
    ```bash
    node <base>/.claude/hooks/openflow-gate.mjs check-design-consistency <变更名>
    ```
  - **不跑 gate 直接判通过 = 闸门 3 未执行。** 每条 warning 的语义由 gate 输出自带，按输出逐条对到改动点上，确认是"改动落错方法"还是"误报"后才放行
  - **warning 为 0 不代表改动点都落地了**（自动检测有启发式上限），下面的逐条核验必须照做
- **改动点逐条核验（必做，最后兜底，输出固定格式）**：对 design「现状与影响面」**每一条声明**（`- 目标：` 与 `- 并行路径：… → 随改`），读对应方法的**当前代码**（读文件本身，不能只看 git diff——已提交的改动 diff 看不到），**必须输出**：
  ```
  改动点 N（声明 {文件路径::方法名}）：代码落点 = {实际方法名}@{行号} → ✅/❌
  ```
  格式硬约束（违反即重做）：
  - **声明 列必须原样抄 design 里的 `` `文件路径::方法名` `` 选择器**，禁止只写方法名、更禁止用功能描述（"填充 XX 字段"这种不算）
  - **代码落点 列必须填调用点**（`this.xxx(...)` / `xxx(...)` 的调用行），禁止填方法定义（`void xxx(...)` 那行是定义不是落点）
  硬规则：
  1. **代码落点方法 ≠ 声明的方法名 → 判「未按设计落地」，直接退回**。反例：声明 `Foo.java::A`，但改动只接进了并行方法 `ANew`——填"存在"不算数，要填"在哪方法"，方法对不上就是失败
  2. **每一条声明单独输出一行落点记录**——只核验一条、或只核验"两条路径状态一致"都不算数
  3. 标注 `不随改` 的并行路径不需要落点记录，但要在 verify-issues.md 记一句理由（声明里的理由可直接引用）
- **用户确认清单（必做，verify 通过的前提）**：逐条核验完成后，把**每一条声明**整理成清单**展示给用户**，**用户显式确认后才算闸门 3 通过**——AI 不能自判"验证通过"：
  | 改动点 | 声明 | 代码落点 | 方法匹配 | **AI 判断依据** | 备注 |
  |--------|------|---------|---------|---------|------|
  | 改动点 3 | `` `Foo.java::A` `` | `` `ANew` ``@行号 | ⚠️ 不匹配 | 声称方法无该改动，改动在并行路径 | gate 判定 ⚠️ |
  硬规则：
  ...
  6. **gate 的每条归属漂移/声称未落地 warning 都必须作为清单里的 ⚠️ 行出现并归属到对应改动点**——AI 不得把它们解释成 ✅、不得删除、不得只说"启发式误报"；要解除 ⚠️ 必须给出**代码证据**（调用点方法名+行号）证明该方法确属另一条已声明的选择器，否则保持 ⚠️。归属漂移若指向并行路径（声明 `Foo.java::A`、落点 `ANew`），必须标 ⚠️ 而不是 ✅
  7. **归属漂移与声称未落地现在是精确判定**（声明带文件归属，gate 不再靠方法名猜），"跨文件同名误报"这个旧借口不成立——报出来就是真的对不上，要么改代码要么改声明
- **design.md 是否含「现状与影响面」章节且改动点声明完整**：缺失或声明格式错 → `check-design-consistency` 直接 blocker，退回 amend 补
...
这一步逐条核对：读取 test-plan.md 中每个场景的原始 scenario 文本（来自 `specs/`），再读取对应测试函数，确认断言方向、**前置条件**和验收条件一致。

- 挑 1-2 个最关键的 scenario 做深度核对（不用全部，但关键路径必须查）
- 核对结果写在 verify-issues.md 中：`T-001 ✅ 断言匹配 / T-002 ⚠️ 测试 check stdout 但 scenario 要求 stderr`
- 发现不匹配 → 回到 build 修正测试

**三个必查项**（每项都要落到 verify-issues.md，不能只看断言文本）：

1. **GIVEN 对齐**：测试的 fixture/setup 是否真的建立了 scenario 的前提？典型事故是 scenario 写 `PAUSED + 待审批`，而夹具硬编码 `NORMAL`——断言再对也测的是另一格。只对比 THEN 不对比 GIVEN，等于没核对
2. **断言有没有失败能力**：这条断言如果被"什么都不做"的实现执行，会不会照样通过？纯 `never()` / `assertNull` / "不抛异常" 形状的断言答案是"会"。`🔴 RED` 标记是它红过的凭据，但凭据只覆盖当时那一版实现——关键路径仍要人眼判一次形状
3. **不存在跨用例委托**：测试注释里出现"真实断言见 T-00x""分叉逻辑在另一个测试验证"就是红旗。委托没有任何机制保证两边的 GIVEN 一致，接缝就是盲区。要么把真断言写回本用例，要么把这层关系升格成 `INV-00x` 不变量行（有 `covers` 子句，gate 会对账）
   - 被 mock 掉的协作者只能证明"委派发生了"，不能证明"委派之后的行为对"。**真实分叉必须在持有真对象的那一侧断言**
...
2. 运行 `write-verify-receipt` 子命令（gate helper 路径定位见主 SKILL.md「状态检测 → Helpers 定位」；脚本缺失或报错 → 停止并要求重装，**没有手动写 receipt 这条路**——手写的 receipt 不带工作树指纹，等于没验证），**input 路径就是上面的 `verify-result.json`**：
...
- **全绿不等于验证过**——绿灯只在断言具备失败能力时才是证据。`🔴 RED` 卡的是这一点，闸门 4 的"断言有没有失败能力"再人工兜一次
