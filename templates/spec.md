...
如果用户在本阶段被打断后继续回复、补充范围、要求调整规格、或确认规格摘要，仍然停留在 spec 阶段。只更新 `openspec/changes/**` 与 `plan-ready.md`，不要修改业务代码。
...
- 本阶段只允许写 `openspec/changes/**` 与 `plan-ready.md`，禁止修改 `src/**`、测试代码或其他业务实现文件
