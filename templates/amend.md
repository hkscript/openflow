...
- **design.md**：**必须同步**——更新「现状与影响面」章节（改动点、链路影响、10 类排查）和 `## 改动文件` 节（实际改动文件，完整路径），新增/删除/修改的文件跟着改；若改动点粒度或生产链路变化，重跑链路追踪
  - **改动点声明行跟着改**：`- 目标：\`文件路径::方法名\`` / `- 并行路径：… → 随改|不随改（理由）`。方法被重命名、移动文件、或改动目标变了，声明必须同步——声明与代码对不上时 `check-design-consistency` 会直接 blocker（精确判定，没有"同名方法顶替"这种模糊空间）
  - 改动点涉及并行路径（带 `New`/`Old`/`V2` 后缀的同前缀兄弟方法）时，同步复查兄弟路径是否也需随改，并把决定写成声明行
...
- 标记为 `🔄 需修改` 的行：保留编号，**清掉行尾的 `🔴 RED ✅ PASS` 后缀**，追加 `⚠️ 待更新: <变更说明>`
  - 为什么必须清 `🔴 RED`：RED 证明的是"这条断言对**当时那版预期行为**有失败能力"。预期行为一改，旧凭据就不再指向新断言——留着等于拿旧收据报新账。断言改完后回 build 重新走 Step 2
- 标记为 `➕ 新增` 的行：追加新行到映射表（不带任何状态后缀）
- 标记为 `❌ 废弃` 的行：保留编号但标记 `❌ 已废弃: <原因>`
- **守卫/状态机被改动时**：复查 `references/code-verification.md`「状态叉乘矩阵」——新增的枚举值或守卫条件会让矩阵长出新的一整列，这些格子必须补 T-id 或补一条 `INV-00x`；已有的 `INV-00x` 行若 `covers` 的场景集合变了，`covers` 子句跟着改（指向不存在的 ID 是 blocker）
