...
2. **读取阶段参考文件**：`brainstorming.md`（与主协调器同目录）
   - 包含 brainstorming 阶段的详细指令和流程

3. **按指令执行**：先检查前置条件，再按 brainstorming.md 的流程执行

4. **设置阶段状态**：确认 `.openflow/phase` 记录当前阶段（`{"version":1,"change":"<变更名>","phase":"brainstorming"}`），缺失则写入（非 build 阶段不带 mode/task，见主协调器「阶段状态」节）。
